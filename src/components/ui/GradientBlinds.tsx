...
export const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("GradientBlinds WebGL init failed:", e);
      return;
    }
...
`;
    ...
    const uniforms = {
      iResolution: {
        value: [gl.drawingBufferWidth || 800, gl.drawingBufferHeight || 600, 1]
      },
      iMouse: { value: [0, 0] },
      iTime: { value: 0 },
    ...
    const resize = () => {
      ...
      const w = Math.max(1, Math.floor(rect.width || container.clientWidth || 800));
      const h = Math.max(1, Math.floor(rect.height || container.clientHeight || 600));
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden absolute inset-0 ${className}`.trim()}
      style={{
